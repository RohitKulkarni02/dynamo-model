...
class VATEXFeaturesDataset(Dataset):
    """VATEX dataset for preprocessed features."""
    
    def __init__(self, features_dir: str, ann_json: str, frames_per_clip: int, sample_strategy: str = "uniform", max_videos: int = None):
        super().__init__()
        ...
        if self.max_videos is not None and len(self.records) > self.max_videos:
            ...
            print(f"VATEXFeaturesDataset: Limited to {self.max_videos} features")
        
        if not self.records:
            raise RuntimeError(f"No usable VATEX features found in {self.features_dir}")
        
        print(f"VATEXFeaturesDataset: {len(self.records)} features found in {self.features_dir}")
    ...
    def __getitem__(self, idx):
        ...
        if total_temporal_steps <= self.frames_per_clip:
            ...
            if self.sample_strategy == "uniform":
                sampled_indices = np.linspace(0, total_temporal_steps - 1, self.frames_per_clip).astype(int)
            else:
...
class VATEXFeaturesDataModule(L.LightningDataModule):
    """VATEX datamodule for preprocessed features."""
